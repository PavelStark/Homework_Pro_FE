// 1. Напишите функцию checkDate() которая в консоль отобразит количество дней до конца текущего месяца.
//    Примечание: инициализация дат должна быть внутри функции

/* function checkDate() {
  console.log(daysInMonth() - new Date().getDate());
  //--------------------------------------------------------------------------
  
  function daysInMonth() {
    let date = new Date();
    let month = date.getMonth() + 1;
    let year = date.getFullYear();
    return new Date(year, month, 0).getDate();
  }
}
checkDate(); */

function checkDate() {
  console.log(new Date(new Date().getFullYear(), new Date().getMonth() +1, 0).getDate() - new Date().getDate());
  
}
checkDate();

/* date = new Date(new Date().getFullYear(), 10 , 0).getDate() 
console.log(date) */

// 2. Создайте функцию isItFridayToday(), которая выводит сообщение согласно условию:
//    Если текущая дата определит пятницу, функция должна в консоль отобразить ответ “Сегодня пятница”!
//    Если пятница была вчера - “Пятница была вчера”
//    Если пятница будет завтра - “Завтра пятница!”
//    Иначе необходимо вывести “Пятница будет через n дней”, где n - это количество дней до пятницы.
//    Предусмотрите вывод 4 пункта (дня, дней)

/* function isItFridayToday() {

  date = new Date(2023, 9, 24, 15);
  console.log(date);
  
  let days = [
    "Пятница будет через 5 дней",
    "Пятница будет через 4 дня",
    "Пятница будет через 3 дня",
    "Пятница будет через 2 дня",
    "Завтра пятница",
    "Сегодня пятница",
    "Пятница была вчера",
  ];
  console.log(days[date.getDay()]);
}
isItFridayToday(); */

//    3. Задан массив prices, передающий элементы строкового типа. Напишите функцию getInfo(prices),
//    которая получает массив и возвращает количество элементов, которые начинаются на “Цена” и
//    количество элементов, которые заканчиваются на “$” в виде массива по примеру:

//    Пример:
let prices = [
  "Цена товара - 1200$",
  "Стоимость - 500$",
  "Цена не определена",
  "9999",
  "Ценовая категория - больше 300$",
  "Цена за услугу 500 EUR",
];

/* console.log(getInfo(prices))
    Результат: [3,3] */

/* function getInfo(prices) {
  acc = 0;
  acc2 = 0;
  array = []

  prices.forEach((elem) => {
    if (elem.startsWith("Цена")) {
      acc++;
    }
    if (elem.endsWith("$")) {
      acc2++;
    }
  })
  array.push(acc, acc2)
  return array;
}

console.log(getInfo(prices)) */

//    4. Задан массив arrays.Сформируйте два новых массива phones и emails, которые будут
//    содержать данные по примеру:

let arrays = [
  "89840959944 exampleonemain@mail.ru",
  "+79840959933 forreason@yandex.net",
  "somemail@mail.ru",
  "89840959900 example@gmil.ru",
  "+79840959911",
  "grandthere@mail.ru",
  "jetpackfor@gmail.ru",
  "89840959922 some@yandex.com",
];

/* Результат: [
  "89840959944",
  "+79840959933",
  "89840959900",
  "+79840959911",
  "89840959922",
][
  ("exampleonemain@mail.ru",
  "forreason@yandex.net",
  "somemail@mail.ru",
  "example@gmil.ru",
  "grandthere@mail.ru",
  "jetpackfor@gmail.ru",
  "some@yandex.com")
];  */

/* let array = []
let array2 = []

for (let i = 0; i < arrays.length; i++) {
  
  array = arrays[i].split(" ")
  array2.push(array) 
}

let emails = array2.flat().filter(elem => elem.includes("."))
let phones = array2.flat().filter(elem => !elem.includes(".")) 
console.log(emails)
console.log(phones) */

// 5. Задан массив cards, передающий элементы строкового типа.Необходимо зашифровать номера карт по
//    следующему правилу: заменить всю строку на 5 символов “*”, оставив первые 4 и последние 4 цифры.

// Пример:
let phones = [
  "4000 0012 0056 9499",
  "4000 0013 5456 7379",
  "4000 0014 1456 9869",
  "4000 0015 3466 7859",
  "4000 0016 3556 6899",
  "4000 0017 4456 4699",
];

/* Результат: [
'4000*****9499',
'4000*****7379',
'4000*****9869',
'4000*****7859',
'4000*****6899',
'4000*****4699'
]  */

/* let array = [];
let array2 = [];
let result = [];

for (let i = 0; i < phones.length; i++) {
  array = phones[i].split(" ");
  array2.push(array);
}

for (let i = 0; i < array2.length; i++) {
  result.push(array2[i][0] + "*****" + array2[i][3]);
}

console.log(result); */
